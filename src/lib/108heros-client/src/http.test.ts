...
// tries it. See #95.
describe("Api108Heros multipart uploads", () => {
  const uploadFieldNames = async (
    call: (client: Api108Heros) => Promise<unknown>,
  ): Promise<string[]> => {
    let fields: string[] = [];
    ...
    const client = new Api108Heros("http://localhost:8536", { fetchFunction });
    await call(client);
    return fields;
  };
  ...
  it.each([
    ["uploadUserAvatar", (c: Api108Heros) => c.uploadUserAvatar({ image: image() })],
    ["uploadUserBanner", (c: Api108Heros) => c.uploadUserBanner({ image: image() })],
    ["uploadSiteIcon", (c: Api108Heros) => c.uploadSiteIcon({ image: image() })],
    ["uploadSiteBanner", (c: Api108Heros) => c.uploadSiteBanner({ image: image() })],
    ["uploadCategoryIcon", (c: Api108Heros) => c.uploadCategoryIcon({ id: 1 }, { image: image() })],
    ["uploadCategoryBanner", (c: Api108Heros) => c.uploadCategoryBanner({ id: 1 }, { image: image() })],
