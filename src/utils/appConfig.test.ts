...
describe("getAppName", () => {
  ...
  // The fallback is the rebranded product name on purpose: 108-Plaza/108jobs-web
  // was swept to 108heros in 660d682 (2026-08-25) and the deployed
  // NEXT_PUBLIC_APP_NAME follows it. The mirror briefly asserted the opposite
  // (dev #140); the owner settled it on 2026-09-09 in favour of the org sweep.
  it("names the product when nothing is configured", () => {
  ...
  it("prefers a configured name", () => {
    process.env.NEXT_PUBLIC_APP_NAME = "Staging 108Heros";

    expect(getAppName()).toBe("Staging 108Heros");
  });
});
